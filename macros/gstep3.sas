...
%macro gstep3(input_ds= /*input data with libname*/
	...
	%do w=1 %to &waven.;
		...
		%do c=1 %to &tvcn.;
			...
			data &con.&waveno._nc(keep=mergeid &con.&waveno. i &groupvar.);
		...
		%do m=1 %to &tvmn.;
			...
			proc logistic inmodel=t.om_&cat.&waveno.;
				...
				out=pom_&cat.&waveno._nc(keep=mergeid p_: censor: i &groupvar.);

				by &groupvar.;
			run;
			
			data &cat.&waveno._nc(keep=mergeid i &cat.&waveno. &groupvar.);
		...
		%do n=1 %to &tvbn.;
			...
			proc logistic inmodel=t.om_&bin.&waveno.;
				...
				%if &waveno.=1 %then %do; &input_ds. %end; 
				%else %do; &lib..w&x._snc %end; 
				out=pom_&bin.&waveno._nc(keep=mergeid p_: censor: i &groupvar.);

				by &groupvar.;
			run;
			
			data &bin.&waveno._nc(keep=mergeid i &bin.&waveno. &groupvar.);
		...
		proc logistic inmodel=t.om_&yvar.&waveno.;
			...
			%if &waveno.=1 %then %do; &input_ds. %end; 
			%else %do; &lib..w&x._snc %end;
			out=pom_&yvar.&waveno._nc(keep=mergeid p_: censor: i &groupvar.);
			by &groupvar.;
		run;
		data &yvar.&waveno._nc(keep=mergeid i &yvar.&waveno. &groupvar.);
		...
		data &lib..w&waveno._snc;
			merge 
				%if &waveno.=1 %then %do; 
					&input_ds.(keep=&groupvar. mergeid i country: gender yedu nchild censor: intercept eurodcat: giv_help0) 
				%end;
				%else %do; 
					&lib..w&x._snc(keep=&groupvar. mergeid i country: gender yedu nchild censor: intercept) 
				%end;
				%do q=1 %to &tvn.; 
					&&tv&q..&waveno._nc 
				%end;
