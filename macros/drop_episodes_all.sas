...
%macro drop_episodes_all;
	...
	proc sort data=share_all;
		by _imputation_ mergeid wave;
	run;

	data share_all (drop=lag:);
		format _imputation_ 8. mergeid $12. hhid $11. country wave giv_help gender 8.;
		set share_all;
		by _imputation_ mergeid;
	...
	proc sort data=share_all;
		by _imputation_ mergeid censor descending wave;
	run;

	*The first observation before death or lost-to-followup will be censored;
	data share_all;
		set share_all;
		by _imputation_ mergeid censor;
		if first.censor & wave ne 8 then censor=1;
	run;
	proc sort data=share_all;
		by _imputation_ mergeid descending censor wave;
	run;
	data share_all;
		set share_all;
		by _imputation_ mergeid;
	...
	run;
	proc sort data=share_all;
		by _imputation_ mergeid descending wave;
	run;
	data share_all;
		set share_all;
		by _imputation_ mergeid;
