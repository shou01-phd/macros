%macro output_results_boot(filepath=	/*output file path*/
	...
	%if &testmode.=1 %then %do;
		data test;
			set &input_ds.(where=(replicate=1 & _imputation_=1));
		run;
		%let input_ds=test;
		%put &input_ds.;
	%end;
	...
	proc logistic data=&input_ds.
		%if &desc.=1 %then %do; descending %end; ;
		%if &diag.=1 %then %do; 
		...
		model &yvar.=wave 
			  ...
			  %else %do; ; %end;
		by &groupvar.;
		output out=num1 (keep=mergeid wave _level_ num &groupvar.) p=num;
	run;
	quit;
	...
	proc logistic data=&input_ds.		
		...
		model &yvar.=wave 
			  ...
			  %else %do; ; %end;
		by &groupvar.;
		output out=denom1 (keep=mergeid wave _level_ denom &groupvar.) p=denom;
	run;
	quit;
	title;
